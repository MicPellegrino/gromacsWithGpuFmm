...
{
    gmx::ForceProviderInput  forceProviderInput(x, *mdatoms, t, box, nb,  stepWork.computeEnergy, stepWork.doNeighborSearch, *cr);
...
{
    gmx::ForceProviderInput  forceProviderInput(x, *mdatoms, t, box, nb, stepWork.computeEnergy, stepWork.doNeighborSearch, *cr);
    /* Start the FMM in this case */
    forceProviders->calculateForces(forceProviderInput, nullptr);
}
              ...
              const DDBalanceRegionHandler             &ddBalanceRegionHandler)
{
    ...
    //printf("Virial = %d  Energy = %d  Neighbor = %d\n", stepWork.computeVirial, stepWork.computeEnergy,  stepWork.doNeighborSearch);
    ...
    if (stepWork.doNeighborSearch)
    {
        ...
        nbv->setAtomProperties(*mdatoms, fr->cginfo, EEL_FMM(inputrec->coulombtype));
    ...
    if (bUseGPU)
    {
        ...
        if (getenv("BOX_OUT") != nullptr)
            printf("step -> %d, BOX\n %e %e %e\n %e %e %e\n %e %e %e\n\n",step,box[0][0],  box[0][1],  box[0][2],  box[1][0],  box[1][1] ,  box[1][2], box[2][0],  box[2][1] ,  box[2][2] );

        if(EEL_FMM(inputrec->coulombtype))
        {
    ...
    if (bUseOrEmulGPU && !alternateGpuWait)
    {
        ...
        if (useGpuFBufOps == BufferOpsUseGpu::True)
        {
            ...
            if (haveLocalForceContribInCpuBuffer && !useGpuForcesHaloExchange)
            {
                ...
                if (getenv("NO_CPU_FORCES") != nullptr)
                {
                    if(int64_t(atoi(getenv("NO_CPU_FORCES"))) == 0)
                    {
                        nbv->launch_copy_f_to_gpu(f, Nbnxm::AtomLocality::Local);
                    }
                    else
                    {
                        nbv->init_f_on_gpu(f, Nbnxm::AtomLocality::Local);
                    }
                }
                else
                {
                    float buff = 0.0;
//#pragma omp parallel for reduction(+:buff) num_threads(gmx_omp_nthreads_get_simple_rvec_task(emntDefault,  forceWithShift.size()))
                    for(int i = 0; i < forceWithShift.size(); i++)
                    {
                        buff += f[i][0];
                        buff += f[i][1];
                        buff += f[i][2];
        ...
        else
        {
            ...
            if(EEL_FMM(inputrec->coulombtype))
            {
                computeSpecialForces(fplog, cr, inputrec, awh, enforcedRotation,
                                     ...
                                     fr->forceProviders, fr->nbv.get(), box, x.unpaddedArrayRef(), mdatoms, lambda.data(),
    ...
    if (getenv("FORCES_OUT") != nullptr)
    {
        if(step == int64_t(atoi(getenv("FORCES_OUT"))))
        {
            gmx::ArrayRef<gmx::RVec>  forceZ = forceOut.forceWithShiftForces().force();
            rvec *f = as_rvec_array(forceZ.data());
            printf("FORCES>\n");
            for(int i = 0; i < forceZ.size(); i++)
            {
                printf("%d %.64e %.64e %.64e\n",i, f[i][0], f[i][1], f[i][2] );
            }
            printf("FORCES<\n");
        }
    ...
    if (getenv("POSITIONS_OUT") != nullptr)
    {
        gmx::ArrayRef<gmx::RVec> positions =  x.unpaddedArrayRef();
        rvec *Xx = as_rvec_array(positions.data());
        printf("XxXxXx\n");
