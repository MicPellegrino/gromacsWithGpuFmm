                                   ...
                                   interaction_const_t *ic)
{
    ...
    // TODO: This needs to be fixed at some point, e.g. by not even constructing
    // coulombEwaldTables in the case of FMM electrostatics
    if (EEL_FMM(ic->eeltype))
    {
        fprintf(stderr, "FMM: As a preliminary hack to suppress 1/scale division by zero in \n"
                "     nonbonded kernels with FMM, setting ic->coulombEwaldTables->scale to unity.\n");
        ic->coulombEwaldTables->scale = 1.0;
    }
}
