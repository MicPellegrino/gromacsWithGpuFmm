...
/** Returns an opaque pointer to the GPU h2d event. For FMM only
 *  Note: CUDA only.
 */
CUDA_FUNC_QUALIFIER
void *gpu_get_gmx_h2d_done_event(gmx_nbnxn_gpu_t gmx_unused     *nb) CUDA_FUNC_TERM_WITH_RETURN(nullptr);
...
CUDA_FUNC_QUALIFIER
void *gpu_get_lj_comb(gmx_nbnxn_gpu_t gmx_unused *nb) CUDA_FUNC_TERM_WITH_RETURN(nullptr);
