                        ...
                        bool                 bFMM)
{
    ...
    if (bFMM)
    {
        // In case of FMM electrostatics, all Coulomb interactions are calculated
        // by the FMM kernels. Pass an empty charge array so that the nbnxn
        // kernels only calculate LJ interactions. 

        real *empty_charge_array;

        //fprintf(stderr, "FMM hack: passing empty charge array to nbnxn_atomdata_set_charges!\n");
        snew(empty_charge_array, mdatoms->nr);

        nbnxn_atomdata_set_charges(nbat, gridSet, empty_charge_array);

        sfree(empty_charge_array);
    }
    else
    {
