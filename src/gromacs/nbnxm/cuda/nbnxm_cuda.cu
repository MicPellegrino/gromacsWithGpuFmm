                             ...
                             const rvec                      *coordinatesHost)
{
    ...
    cudaEventRecord(nb->gmx_h2d_done, stream);
...
void nbnxn_init_f_on_gpu(const AtomLocality               atomLocality,
                     const Nbnxm::GridSet            &gridSet,
                     gmx_nbnxn_gpu_t                 *nb,
                     rvec                            *f)
{
    ...
    cudaMemsetAsync(ptrDest, 0.0, numCopyAtoms*sizeof(rvec), stream);
