                         ...
                         const nbnxn_atomdata_t::Params &nbatParams)
{
    ...
    if (ic->eeltype == eelCUT || EEL_FMM(ic->eeltype)) //hackhackhack
    {
    ...
    else if (EEL_FMM(ic->eeltype))
    {
        // In case of FMM electrostatics, 
        // Coulomb interactions are switched off in nbnxn kernels
        fprintf(stderr, "init_nbparam: Using FMM electrostatics.\n");
    }
    else
    {
         ...
         gmx_bool                   bLocalAndNonlocal)
{
    ...
    stat = cudaEventCreateWithFlags(&nb->gmx_h2d_done, cudaEventDisableTiming);
    CU_RET_ERR(stat, "cudaEventCreate on gmx_h2d_done failed");
...
void gpu_free(gmx_nbnxn_cuda_t *nb)
{
    ...
    stat = cudaEventDestroy(nb->gmx_h2d_done);
    CU_RET_ERR(stat, "cudaEventDestroy failed on timers->gmx_h2d_done");
...
void *gpu_get_gmx_h2d_done_event(gmx_nbnxn_gpu_t *nb)
{
    assert(nb);

    return static_cast<void *>(&nb->gmx_h2d_done);
}
...
void *gpu_get_lj_comb(gmx_nbnxn_gpu_t *nb)
{
    assert(nb);

    return static_cast<void *>(nb->atdat->lj_comb);
}
