...
class ForceProviderInput
{
    public:
         ...
         * \param[in]  calcEnergies  Should the force provider also calculate energies at this time step?
        ...
        ForceProviderInput(ArrayRef<const RVec> x,
                           ...
                           gmx_bool             calcEnergies,
                           gmx_bool             doNeighborSearching,
                           const t_commrec     &cr)
            : x_(x), mdatoms_(mdatoms), t_(time), cr_(cr), calculateEnergies_(calcEnergies), doNeighborSearching_(doNeighborSearching), nb_(nb)
        {
            copy_mat(box, box_);
        }
        ...
        gmx_bool             calculateEnergies_;                        //!< Whether or not energies are needed
        gmx_bool             doNeighborSearching_;
        nonbonded_verlet_t  *nb_;
};
